    ...
    for module_name in ["create", "update", "delete", "get"]:
        ...
        for key, value in list(module.__dict__.items()):
            if not key.startswith("_") and (
                hasattr(value, "__call__") and (value.__module__ == module_path)
            ):
                functions[key] = value
    ...
    if filter_name in fields:
        ...
        sql_string = """SELECT DISTINCT "{column}"
         FROM "{resource}" {where}""".format(
            column=filter_name, resource=resource_id, where=where_clause
        )
...
def get_color_scheme():
    ...
    """
    colors = [
        {"value": "#59a14f", "text": _("Green")},
        {"value": "#4e79a7", "text": _("Blue")},
        {"value": "#499894", "text": _("Teal")},
        {"value": "#b6992d", "text": _("Golden")},
        {"value": "#ffa600", "text": _("Yellow")},
        {"value": "#d87c26", "text": _("Orange")},
        {"value": "#9d7660", "text": _("Brown")},
        {"value": "#78549a", "text": _("Purple")},
        {"value": "#b2182b", "text": _("Red")},
    ]
...
def get_map_color_scheme():
    ...
    colors = [
        ...
        {"value": "#7b3294,#c2a5cf,#EFD9CE,#a6dba0,#008837", "text": _("Green-Purple")},
        {"value": "#d7191c,#fdae61,#ffffbf,#abdda4,#2b83ba", "text": _("Blue-Red")},
        {"value": "#a6611a,#dfc27d,#FCD0A1,#80cdc1,#018571", "text": _("Teal-Brown")},
        {
...
def get_charts_data_formats(num=None):
    ...
    options = [
        ...
        {"text": _("Percentage (0 digit) e.g 25%% for 0.25"), "value": ".0%"},
        {"text": _("Percentage (1 digit) e.g 25.1%% for 0.251"), "value": ".1%"},
        {"text": _("Percentage (2 digit) e.g 25.12%% for 0.2512"), "value": ".2%"},
        {"text": _("Comma thousands separator (0 digit) e.g 2,512"), "value": ",.0f"},
        {"text": _("Comma thousands separator (1 digit) e.g 2,512.3"), "value": ",.1f"},
        {
            "text": _("Comma thousands separator (2 digit) e.g 2,512.34"),
            "value": ",.2f",
        },
        {"text": _("Date DD/MM/YYYY .e.g. 01/01/2014"), "value": "%d/%m/%Y"},
        {"text": _("Date YYYY/MM/DD .e.g. 2014/01/01"), "value": "%Y/%m/%d"},
        {"text": _("Date YYYY/MM .e.g. 2014/01"), "value": "%Y/%m"},
        {"text": _("Date MM/YYYY .e.g. 01/2014"), "value": "%m/%Y"},
        {"text": _("Date DD/MM .e.g. 01/01"), "value": "%d/%m"},
        {"text": _("Date MM/YY .e.g. 01/14"), "value": "%m/%y"},
        {"text": _("Date YYYY .e.g. 2014"), "value": "%Y"},
        {"text": _("Date MMM DD, YYYY .e.g. Jan 01, 2014"), "value": "%b %d, %Y"},
        {"text": _("Date MMM DD .e.g. Jan 01"), "value": "%b %d"},
        {"text": _("Date MMM YY .e.g. Jan 14"), "value": "%b %y"},
    ]
    if num:
        return options[:num]
    return options
...
def get_maps_data_formats():
    options = [
        ...
        {"text": _("Decimal (3 digit) e.g 2.501"), "value": ".3f"},
        {"text": _("Decimal (4 digit) e.g 2.5012"), "value": ".4f"},
        {"text": _("Currency e.g. $2,000"), "value": "$,.2f"},
...
def get_visualization_size():
    ...
    """
    options = [
        {"text": _("Small Rectangle (1x2)"), "value": "size-sm"},
        ...
        {"text": _("Large Rectangle (2x4)"), "value": "size-sm double square"},
    ...
    if any(filters):
        if len(filters) > 1:
            ...
            for idx, _ in enumerate(filters):
                ...
                if idx == 0:
                    where_clause = "WHERE (\"{0}\" {1} '{2}')".format(name, op, value)
                else:
                    where_clause += " AND (\"{0}\" {1} '{2}')".format(name, op, value)
...
def get_resource_columns(res_id, escape_columns=[]):
    ...
    """
    data = {"resource_id": res_id, "limit": 0}
...
def _get_s3_presigned_url(url):
    ...
    try:
        ...
        s3_vars = [bucket_name, p_key, s_key, host_name]

        if not all(s3_vars):
            log.error("S3 configuration is missing")
            return url, "S3 configuration is missing"

        s3_key_res_id = url.split("resource/")[1].split("/")[0]
        s3_key_file_name = url.split("/")[-1]
        s3_key = f"resources/{s3_key_res_id}/{s3_key_file_name}"
        ...
        log.error(e)
        error = (
            f"Error getting S3 presigned URL. Attempting to use the original URL: {url}"
        )
...
def get_geojson_properties(url):
    ...
    ckan_plugins = config.get("ckan.plugins", "")

    if "s3filestore" in ckan_plugins:
        url, error = _get_s3_presigned_url(url)
    ...
    for k, v in geojson.get("features")[0].get("properties").items():
...
def get_map_data(
    geojson_url, map_key_field, data_key_field, data_value_field, from_where_clause
):
    geojson_keys = []
    ...
    if "s3filestore" in ckan_plugins:
        geojson_url, error = _get_s3_presigned_url(geojson_url)
    ...
    response = toolkit.get_action("datastore_search_sql")(context, {"sql": sql})
    ...
    for record in records_to_lower:
        ...
        mapping[key] = {"key": key, "value": value}

    map_data = {"geojson_data": geojson_data, "features_values": mapping}
...
def get_available_groups(group_name=None):
    ...
    groups = [
        {"text": "", "value": ""},
    ] + [{"text": group.get("title"), "value": group.get("name")} for group in groups]
...
def get_map_config():
    ...
    map_config = {
        "osm_url": config.get(
            "ckanext.querytool.map_osm_url",
            "https://cartodb-basemaps-{s}.global.ssl"
            ".fastly.net/"
            "light_nolabels/{z}/{x}/{y}{r}.png",
        ),
        "osm_attribute": config.get(
            "ckanext.querytool.map_osm_attribute",
            "&copy; <a href="
...
def parse_y_axis_columns(value):
    ...
    # Legacy
        ...
        names = value.split(",")
        columns = [{"name": name, "alias": ""} for name in names]
        return columns
...
def sort_by_orphaned_reports(reports, q=None):
    ...
    if q:
        ...
        sorted_reports = [
            querytool
            for querytool in sorted_reports
            if querytool["name"] in querytool_search_results_names
        ]
...
def get_is_admin_or_editor_of_any_group(userobj):
    ...
    is_admin_or_editor = [
        get_user_permission_type(userobj, group["id"]) for group in groups
    ]
...
def get_all_org_permissions(userobj):
    ...
    permissions = [get_user_permission_type(userobj, org.get("id")) for org in orgs]
...
def get_cookie_control_config():
    ...
    enabled = config.get("ckanext.querytool.cc.enabled", "false").lower()
    enabled = enabled if enabled in ["true", "false"] else "false"
    cookie_control_config["enabled"] = enabled

    api_key = config.get("ckanext.querytool.cc.api_key", "")
    cookie_control_config["api_key"] = api_key

    license_type = config.get("ckanext.querytool.cc.license_type", "")
    cookie_control_config["license_type"] = license_type

    popup_position = config.get("ckanext.querytool.cc.popup_position", "")
    cookie_control_config["popup_position"] = popup_position

    theme_color = config.get("ckanext.querytool.cc.theme_color", "")
    cookie_control_config["theme_color"] = theme_color

    initial_state = config.get("ckanext.querytool.cc.initial_state", "")
    ...
    text = json.dumps(
        {
            ...
            "necessary_title": config.get(
                "ckanext.querytool.cc.text.necessary_title", ""
            ),
            "necessary_description": config.get(
                "ckanext.querytool.cc.text.necessary_description", ""
            ),
            "analytical_title": config.get(
                "ckanext.querytool.cc.text.analytical_title", ""
            ),
            "analytical_description": config.get(
                "ckanext.querytool.cc.text.analytical_description", ""
            ),
        }
    )
...
def get_social_media_links(social_order, admin_dict):
    ...
    for sm in social_order:
        try:
            ...
            continue
        if sm_link:
            social_media_links.append({"type": sm, "link": sm_link})
...
def querytool_search(query_string=None, query_group=None):
    ...
    return list(
        set(querytool.search(query_string=query_string, query_group=query_group))
    )
    ...
    if remove_private is False:
        ...
        report_count = len(
            [
                r
                for r in reports
                if r.get("private") is False and r.get("type") == "main"
            ]
        )
...
def filter_reports_by_permissions(reports, for_groups=False, for_sorting=False):
    ...
    if for_sorting is True:
        filtered_reports = [
            report for report in reports if report["group"] in user_groups
        ]
        ...
        for report in reports:
            if report not in filtered_reports:
                try:
                    ...
                    if group["state"] == "deleted" and report["owner_org"] in [
        ...
        filtered_reports = [
            report for report in reports if report["name"] in user_groups
        ]
...
def group_count_design_reports(current_c):
    reports = filter_reports_by_permissions(current_c.page.items, for_groups=True)
...
def color_to_hex(color):
    ...
    if color in list(named_colors.keys()):
