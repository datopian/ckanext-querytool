...
ckan.module("querytool-viz-preview", function () {
  var api = {
    ...
    post: function (action, data) {
      ...
      //Write a post request application/json
      return $.ajax({
        type: "POST",
        contentType: "json",
  ...
  return {
    ...
    create_sql: function () {
      ...
      var chart_filter_name =
        this.options.filter_name === true ? "" : this.options.filter_name; //  NOTE: mapped to e
      var chart_filter_value =
        this.options.filter_value === true ? "" : this.options.filter_value; //  NOTE: mapped to n
      var y_axis = this.options.y_axis === true ? "" : this.options.y_axis; //  NOTE: mapped to i
      var static_reference_columns =
        this.options.static_reference_columns === true
          ? []
          : this.options.static_reference_columns;
      var static_reference_column = this.getStaticReferenceColumn(
        static_reference_columns,
        y_axis,
      ); //  NOTE: mapped to o
      var category =
        this.options.category_name === true ? "" : this.options.category_name; //  TODO: remove this line, `category` ins't used anywhere
      ...
      if (chart_filter_name && chart_filter_value) {
        sqlStringExceptSelect +=
          ' AND ("' + tmp_filter_name + '"' + " = '" + tmp_filter_value + "')"; //  NOTE: mapped to t
      }
      ...
      if (static_reference_column) {
        ...
        if (
          ...
          sql =
            'SELECT AVG("' +
            static_reference_column +
            '") as static_reference_column, "' +
            this.options.x_axis +
            '", SUM("' +
            upper_bounds +
            '") as "' +
            upper_bounds.toLowerCase() +
            '", SUM("' +
            lower_bounds +
            '") as "' +
            lower_bounds.toLowerCase() +
            '", SUM("' +
            this.options.y_axis +
            '") as "' +
            this.options.y_axis +
    ...
    get_resource_datа: function (sql) {
      ...
      var upper_bounds =
        this.options.upper_bounds === true ? "" : this.options.upper_bounds;
      var lower_bounds =
        this.options.lower_bounds === true ? "" : this.options.lower_bounds;
      ...
      var chart_filter_name =
        this.options.filter_name === true ? "" : this.options.filter_name; // NOTE: mapped to s
      var chart_filter_value =
        this.options.filter_value === true ? "" : this.options.filter_value; // NOTE: mapped to c
      var static_reference_columns =
        this.options.static_reference_columns === true
          ? []
          : this.options.static_reference_columns; // NOTE: mapped to u
      var static_reference_column = this.getStaticReferenceColumn(
        static_reference_columns,
        y_axis,
      ); // NOTE: mapped to l
      var dynamic_reference_type =
        this.options.dynamic_reference_type === true
          ? ""
          : this.options.dynamic_reference_type; // NOTE: mapped to f
      var dynamic_reference_factor =
        this.options.dynamic_reference_factor === true
          ? ""
          : this.options.dynamic_reference_factor; // NOTE: mapped to p

      // @ts-ignore
      var viz_form = $("#visualizations-form");
      var f = viz_form.data("mainFilters"); // NOTE: mapped to h
      var previous_filters =
        this.options.query_filters === true ? f : this.options.query_filters; // NOTE: mapped to _
      ...
      if (chart_filter_name && chart_filter_value) {
        chart_filter = {
          ...
          value:
            typeof chart_filter_value === "string"
              ? // @ts-ignore
              ...
              function removeCustomSorting(fetchedData) {
                for (var key in fetchedData) {
                  if (fetchedData.hasOwnProperty(key)) {
                    ...
                    name = name.replace(/^"|"$/g, "").replace(/^\d{1,2}\./, "");
              ...
              if (static_reference_column) {
                if (category) {
                  this.static_reference_value =
                    this.fetched_data.static_reference_value;
                  ...
                  this.static_reference_value =
                    static_reference_values.reduce(function (a, b) {
                      return a + b;
                    }, 0) / static_reference_values.length;
                }
              }
              ...
              if (dynamic_reference_type) {
                ...
                if (dynamic_reference_factor !== "") {
                  this.dynamic_reference_value =
                    this.dynamic_reference_value * dynamic_reference_factor;
                }
              }
    ...
    createChart: function (data) {
      ...
      var chart_padding_left =
        this.options.chart_padding_left === true
          ? null
          : this.options.chart_padding_left;
      var chart_padding_bottom =
        this.options.chart_padding_bottom === true
          ? null
          : this.options.chart_padding_bottom;
      var padding_top =
        this.options.padding_top === true ? null : this.options.padding_top;
      var padding_bottom =
        this.options.padding_bottom === true
          ? null
          : this.options.padding_bottom;
      var tick_count =
        this.options.tick_count === true ? "" : this.options.tick_count;
      ...
      var upper_bounds =
        this.options.upper_bounds === true
          ? ""
          : this.options.upper_bounds.toLowerCase();
      var lower_bounds =
        this.options.lower_bounds === true
          ? ""
          : this.options.lower_bounds.toLowerCase();
      var show_bounds = this.options.show_bounds; //  NOTE: mapped to bnds
      var additionalCategory =
        this.options.category_name === true ? "" : this.options.category_name; //  NOTE: mapped to g
      var static_reference_label =
        this.options.static_reference_label === true
          ? ""
          : this.options.static_reference_label;
      var dynamic_reference_label =
        this.options.dynamic_reference_label === true
          ? ""
          : this.options.dynamic_reference_label;
      var x_sort_labels = this.options.x_sort_labels;
      var values;
      var show_labels_as_percentages =
        this.options.show_labels_as_percentages || false;
      var desc =
        this.options.description === true ? "" : this.options.description; //  NOTE: mapped to desc
      ...
      var queryFilters =
        this.options.query_filters === true ? [] : this.options.query_filters; //  NOTE: mapped to E
      var plotly = this.options.plotly === true ? "" : this.options.plotly;
      if (!queryFilters.length)
        queryFilters =
          this.options.info_query_filters === true
            ? []
            : this.options.info_query_filters;
      var optionalFilterName =
        this.options.filter_name === true ? "" : this.options.filter_name;
      var optionalFilterSlug =
        this.options.filter_slug === true ? "" : this.options.filter_slug;
      var optionalFilterValue =
        this.options.filter_value === true ? "" : this.options.filter_value; //  NOTE: mapped to N
      ...
      if (
        ...
        (this.options.category_name === "" ||
          typeof this.options.category_name === "boolean")
      ) {
      ...
      // Reference lines
      if (
        !["sbar", "shbar", "donut", "pie"].includes(this.options.chart_type)
      ) {
        ...
        if (this.static_reference_value || this.dynamic_reference_value) {
          options.axis.y.min = Math.min.apply(
            null,
            [
              this.static_reference_value,
              this.dynamic_reference_value,
          ...
          options.axis.y.max = Math.max.apply(
            null,
            [
              this.static_reference_value,
              this.dynamic_reference_value,
      ...
      if (
        [
          ...
          "sbar",
          "shbar",
        ].includes(this.options.chart_type)
      ) {
        if (y_from_zero) {
      ...
      function dateWidthConvert(xValues, barWidth) {
        if (
          Array.from(xValues).every(
            (i) => new Date(i) !== "Invalid Date" && !isNaN(new Date(i)),
          )
        ) {
        ...
        for (tmp = len_data - 1; tmp >= 0; tmp--) {
          ...
          if (elementExists) {
            ...
            if (html_line_type == "solid") {
              ...
              html +=
                '<option value="solid">' + i18n[lang]["Solid"] + "</option>";
            }

            if (html_line_type == "dash") {
              html +=
                '<option value="dash" selected>' +
                i18n[lang]["Dashed"] +
                "</option>";
            } else {
              html +=
                '<option value="dash">' + i18n[lang]["Dashed"] + "</option>";
            }

            if (html_line_type == "dashdot") {
              html +=
                '<option value="dashdot" selected>' +
                i18n[lang]["Dash Dot"] +
                "</option>";
            } else {
              html +=
            ...
            if (html_line_type == "dot") {
              ...
              html +=
                '<option value="dot">' + i18n[lang]["Dotted"] + "</option>";
            }
            ...
            if (
              ...
              html +=
                '<option value="4">' + i18n[lang]["Regular"] + "</option>";
            }
      ...
      if (typeof plotly === "string" || plotly === true) {
        ...
        if (typeof item_exists !== "undefined" && item_exists !== null) {
          ...
          var colorInputs = $(`input[id^="chart_field_color_${item_no}"]`);
          ...
          for (tmp = len_data - 1; tmp >= 0; tmp--) {
            ...
            //  Get color input
            var colorInputSelector =
              "#chart_field_color_" + item_no + "_" + (tmp + 1);
      ...
      if (
        ...
        data.forEach((data_el, idx) => {
          ...
          let sortFn = isNaN(Date.parse(data[idx].x[0]))
            ? stringSortFn
            : dateSortFn;
      ...
      if (show_legend_title && show_legend) {
        ...
        else legend_title_text = default_title;
      }
      ...
      if (!["donut", "pie"].includes(chartType)) {
        ...
        if (dateSymbols.some((f) => x_tick_format.includes(f))) {
          ...
          let tmpData = !["hbar", "shbar"].includes(chartType)
            ? data[0].x
            : data[0].y;
          let sample = tmpData[0];

          const is4DigitN = (val) => {
            return typeof val === "number" && val >= 1000 && val <= 9999;
          };
      ...
      if (
        ...
        if (
          ...
          for (j = 0; j < sequential_colors.length; j++) {
      ...
      if (["bar", "sbar", "hbar", "shbar"].includes(this.options.chart_type)) {
        if (show_annotations == true) {
          const isHorizontal = ["hbar", "shbar"].includes(
            ...
            for (var j = 0; j < data[i].x.length; j++) {
              ...
              if (annotationText != "NaN") {
                ...
                if (isHorizontal) {
                  ...
                  annotationData.y = !isStacked
                    ? getAnnotationPos(i, j)
                    : data[i].y[j];
                  annotationData.xanchor = "left";
                  annotationData.yanchor = "center";
                } else {
                  annotationData.x = !isStacked
                    ? getAnnotationPos(i, j)
                    : data[i].x[j];
      ...
      if (
        ...
        if (
          ...
          records[upper_bounds] &&
          records[lower_bounds]
        ) {
          for (var i = 0; i < data.length; i++) {
            if (records[upper_bounds][data[i].name] !== undefined) {
              ...
              for (var j = 1; j < records[data[i].name].length; j++) {
                upper.push(
                  Math.round(
                    Math.abs(
                      records[data[i].name][j] -
                        records[upper_bounds][data[i].name][j - 1],
                    ),
                  ),
                );
                lower.push(
                  Math.round(
                    Math.abs(
                      records[data[i].name][j] -
                        records[lower_bounds][data[i].name][j - 1],
                    ),
                  ),
                );
              }
    ...
    updateChart: function () {
      ...
      var chartFieldSeqColor = chartField
        .find("[name*=seq_colors_hidden_input_]")
        .val(); //  NOTE: mapped to nn
      var chartFieldColorType = chartField
        .find("[name*=chart_field_color_type_]")
        .val(); //  NOTE: mapped to nnn

      var colorSelect = chartField.find("[name*=chart_field_color_]");
      var colorValue = colorSelect.val();

      var chartPaddingLeft = chartField.find(
        "input[name*=chart_field_chart_padding_left_]",
      );
      var chartPaddingLeftVal = chartPaddingLeft.val();

      var chartPaddingBottom = chartField.find(
        "input[name*=chart_field_chart_padding_bottom_]",
      );
      ...
      var legendTitleCkb = chartField.find(
        "input[name*=chart_field_leg_title_]",
      );
      ...
      var xTextMultiline = chartField.find(
        "[name*=chart_field_x_text_multiline_]",
      );
      var xTextMultilineVal = xTextMultiline.is(":checked");

      var chartFieldShowAnnotations = chartField
        .find("input[name*=chart_field_show_annotations_]")
        .is(":checked"); //  NOTE: mapped to sa

      var xTickCullingMax = chartField.find(
        "[name*=chart_field_x_tick_culling_max_]",
      );
      var xTickCullingMaxVal = xTickCullingMax.val();

      var tooltipName = chartField.find(
        "input[name*=chart_field_tooltip_name_]",
      );
      ...
      var xTickFormatVal = chartField
        .find("[name*=chart_field_x_ticks_format_]")
        .val(); //  NOTE: mapped to xf

      var paddingTop = chartField.find("input[name*=chart_field_padding_top_]");
      var paddingTopVal = paddingTop.val();

      var paddingBottom = chartField.find(
        "input[name*=chart_field_padding_bottom_]",
      );
      ...
      var yLabelHide = chartField.find(
        "input[name*=chart_field_y_label_hide_]",
      );
      var yLabelHideVal = yLabelHide.is(":checked");

      var xLabelVal = chartField
        .find("textarea[name*=chart_field_x_label_]")
        .val(); //  NOTE: mapped to xl
      var xLabelHideVal = chartField
        .find("input[name*=chart_field_x_label_hide_]")
        .is(":checked"); //  NOTE: mapped to xlh

      var yFromZero = chartField.find("input[name*=chart_field_y_from_zero_]");
      var yFromZeroVal = yFromZero.is(":checked");

      var axisRange = chartField
        .find("input[name*=chart_field_axis_range_]")
        .is(":checked"); //  NOTE: mapped to yp
      var xFromZeroVal = chartField
        .find("input[name*=chart_field_x_from_zero_]")
        .is(":checked"); //  NOTE: mapped to xfz

      var staticReferenceColumns = chartField.find(
        "select[name*=chart_field_static_reference_columns_]",
      );
      var staticReferenceColumnsVal = staticReferenceColumns.val();

      var staticReferenceLabel = chartField.find(
        "input[name*=chart_field_static_reference_label_]",
      );
      var staticReferenceLabelVal = staticReferenceLabel.val();

      var upperBounds = chartField
        .find("[name*=chart_field_upper_bounds_]")
        .val(); //  NOTE: mapped to ub
      var lowerBounds = chartField
        .find("[name*=chart_field_lower_bounds_]")
        .val(); //  NOTE: mapped to lb
      ...
      var showBounds = chartField
        .find("input[name*=chart_field_show_bounds_]")
        .is(":checked"); //  NOTE: mapped to bnds

      var dynamicReferenceType = chartField.find(
        "select[name*=chart_field_dynamic_reference_type_]",
      );
      var dynamicReferenceTypeVal = dynamicReferenceType.val();

      var dynamicReferenceFactor = chartField.find(
        "input[name*=chart_field_dynamic_reference_factor_]",
      );
      var dynamicReferenceFactorVal = dynamicReferenceFactor.val();

      var dynamicReferenceLabel = chartField.find(
        "input[name*=chart_field_dynamic_reference_label_]",
      );
      var dynamicReferenceLabelVal = dynamicReferenceLabel.val();

      var x_sort_labels = chartField
        .find("input[name*=chart_field_x_sort_labels_]")
        .is(":checked");

      var measureLabelVal = $("#choose_y_axis_column option:selected").text();

      var showLabelsAsPercentages = chartField.find(
        "[name*=chart_field_show_labels_as_percentages_]",
      );
      ...
      var lplotly = chartField
        .find("input[name*=chart_field_plotly_line_]")
        .val();
      var bar_width = chartField
        .find("input[name*=chart_field_bar_width_]")
        .val();
      var donut_hole = chartField
        .find("input[name*=chart_field_donut_hole_]")
        .val();
      ...
      for (let i = 0; i < lwidths_list.length; i++) {
        lwidths.push(
          String(chartField.find(`[name=${lwidths_list[i].id}]`).val()),
        );
      }
    ...
    renderChartTitle: function (title, options) {
      ...
      if (options.optionalFilter)
        data.optional_filter = options.optionalFilter.value;
