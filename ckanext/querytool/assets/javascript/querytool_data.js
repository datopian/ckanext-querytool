(function (_, jQuery) {
  ...
  var api = {
    ...
    getTemplate: function (filename, params, success, error) {
      ...
      var url = ckan.url(
        locale + "/api/1/util/snippet/" + encodeURIComponent(filename)
      );
  ...
  function handleRenderedFilters(item_id, resource_id) {
    ...
    filter_name_select.change(function (event) {
      ...
      var filter_alias_input_id = filter_value_select_id.replace(
        "value",
        "alias"
      );
      var resource_input_id = filter_name_select_id.replace(
        "data_filter_name",
        "resource_id"
      );
    ...
    filter_value_select.mousedown(function (event) {
      ...
      var filter_name_select_id = filter_value_select_id.replace(
        "value",
        "name"
      );
      var filter_name = $("#" + filter_name_select_id)
        .find(":selected")
        .val();
  ...
  function handleRenderedQuerytools(item_id) {
          ...
          if (data.result.length) {
            ...
            $.each(data.result, function (idx, elem) {
              if (selected == elem.name) {
                $("#" + querytool_name_select_id).append(
                  new Option(elem.name, elem.name, false, true)
                );
              } else {
                $("#" + querytool_name_select_id).append(
                  new Option(elem.name, elem.name)
                );
              }
            });
          }
    ...
    querytool_name_select.change(function (event) {
      var elem = $(this);
      var querytool_name_select_id = elem.attr("id");
      var edit_querytool_btn_id = querytool_name_select_id.replace(
        "field-related-querytool",
        "edit-querytool-item-btn"
      );
  ...
  function _getSelectedQuerytools(querytool_item_id) {
    ...
    $.each(querytool_items, function (idx, elem) {
      if ($(elem).attr("id") != querytool_item_id) {
        var querytool = $(elem)
          .find("[id*=field-related-querytool_]")
          .find(":selected")
          .val();
  ...
  $(document).ready(function () {
    ...
    datasetField.change(function (event) {
      ...
      yAxisColumnsNotice.text(
        _("Please choose a resource to see available numeric columns.")
      );
    ...
    add_related_querytool.click(function (event) {
      ...
      if (querytool_items.length < 3) {
        ...
        api
          .getTemplate("related_querytool_item.html", {
            n: total_querytools,
          })
          .done(function (data) {
      ...
      if (dataset_name) {
        api
          ...
          .done(function (data) {
            ...
            chartResourceSelect.append(
              $("<option></option>")
                .attr("value", "")
                .text(_("-- Choose resource --"))
            );

            $.each(dataset_resources, function (i, res) {
              var name = res.name || _("Unnamed resource");
              chartResourceSelect.append(
                $("<option></option>").attr("value", res.id).text(name)
              );
            });
          });
      }
    ...
    function populateYAxisColumns(value) {
      api
        .get("querytool_get_numeric_resource_columns", {
          res_id: value,
        })
              ...
              response.result.forEach(function (item, i) {
                ...
                var element = [
                  ...
                  '<input name="y_axis_name_' +
                    ...
                    item +
                    '" />',
                  '<label class="control-label" for="y_axis_name_' +
                    columnId +
                    '">' +
                    item +
                    "</label>",
                  '<input name="y_axis_alias_' +
                    columnId +
                    '" id="y_axis_alias_' +
                    columnId +
                    '" type="text" placeholder="Optional label" value="">',
                  "</div>",
                ].join("");
